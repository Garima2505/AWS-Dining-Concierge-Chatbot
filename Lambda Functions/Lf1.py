...
def validate_order_restaurants(location, cuisine, date, time, phone_number, no_of_people):
    ...
    if date:
        print(datetime.datetime.strptime(date, '%Y-%m-%d').date() == datetime.date.today())
    ...
    if no_of_people and no_of_people < 0:
        return build_validation_result(False, 'No_of_people',
                                       'The number of people cannot be less than 1. '
...
def dining_suggestions(intent_request):
    ...
    queue = sqs.get_queue_by_name(QueueName='message-queue')
    test_body = {
        "Date": str(date),
        "Cuisine": str(cuisine),
